...
describe 'Medium registration' do

  before do
    @user = FactoryGirl.create(:user, { email: 'abc@example.com', password: 'password'})
    @user2 = FactoryGirl.create(:user, { email: 'qwe@example.com', password: 'password', medium_registration: true})
  end

  it 'Mediumform login with no medium_registration flag', :mediumform do
    visit '/login'
    sign_in(@user)
    visit '/login'
  ...
  it 'Mediumform login with medium_registration flag', :mediumform do
    visit '/login'
    sign_in(@user2)
    visit '/login'
    expect(page).to have_content 'Medium Registration'
  end

  it 'Mediumform form shows', :mediumform do
    visit '/login'
    sign_in(@user2)
    visit '/login'
    click_link('Medium Registration Form')
    expect(page).to have_content 'Your responses will determine your eligibility to participate in this program.'
  end

  it 'Mediumform signature_checkbox need to be checked', :mediumform do
    visit '/login'
    sign_in(@user2)
    visit '/login'
    click_link('Medium Registration Form')
